...
/**
 * Sends a premium styled HTML order status update email to the customer.
 * @param {Object} order - The populated Order document from MongoDB.
 */
export const sendStatusUpdateEmail = async (order) => {
  try {
    const { contact, shippingDetails, orderItems, totalPrice, _id, status, paymentMethod } = order;
    
    // We only send email updates for dispatched or delivered
    ...
    // Configure content based on status
    let statusTitle = '';
    let statusSubtext = '';
    let statusDescription = '';
    let emailSubject = '';
    let trackButtonText = 'Track Your Order';

    if (status === 'dispatched') {
      emailSubject = `Your Order #${orderIdStr} is on the way! | GenZ Skin Studio`;
      statusTitle = 'Order Dispatched';
      statusSubtext = 'ORDER DISPATCHED & ON THE WAY';
      statusDescription = `Great news! We have dispatched your order. It is now in transit and should arrive at your delivery address in 3-5 business days. You can track its live status below.`;
    } else if (status === 'delivered') {
      emailSubject = `Your Order #${orderIdStr} has been delivered! | GenZ Skin Studio`;
      statusTitle = 'Order Delivered';
      statusSubtext = 'ORDER DELIVERED';
      statusDescription = `Your package from GenZ Skin Studio has been successfully delivered. We hope you love your new skin essentials! Thank you for shopping with us.`;
      trackButtonText = 'View Order Details';
    }
              ...
              <table border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.03); border: 1px solid #eaeaea;">
                ...
                <tr>
                  <td align="center" style="background-color: #000000; padding: 35px 40px; color: #ffffff;">
                    ...
                    <p style="font-size: 10px; text-transform: uppercase; letter-spacing: 2px; color: #cda250; font-weight: 700; margin: 8px 0 0 0;">${statusSubtext}</p>
                  </td>
                </tr>
                ...
                <tr>
                  <td style="padding: 40px 40px 20px 40px;">
                    <h1 style="font-size: 20px; font-weight: 700; color: #1a1a1a; margin: 0 0 10px 0;">Hello ${customerName},</h1>
                    <p style="font-size: 14px; line-height: 1.6; color: #555555; margin: 0; font-weight: 300;">${statusDescription}</p>
                  </td>
                </tr>
                      ...
                      <tr>
                        <td>
                          <table border="0" cellpadding="0" cellspacing="0" width="100%" style="font-size: 13px;">
                            <tr>
                              <td style="padding: 4px 0; color: #7a7a7a; font-weight: 500;">Order ID</td>
                            ...
                            <tr>
                              <td style="padding: 4px 0; color: #7a7a7a; font-weight: 500;">Current Status</td>
                              <td style="padding: 4px 0; text-align: right; color: ${status === 'delivered' ? '#2e7d32' : '#d48e00'}; font-weight: 700; text-transform: uppercase; font-size: 11px; letter-spacing: 0.5px;">${status}</td>
                            </tr>
                          </table>
                        </td>
                      </tr>
                ...
                <tr>
                  <td align="center" style="padding: 10px 40px 40px 40px;">
                    <a href="${baseUrl}/track?orderId=${order._id}" style="background-color: #000000; color: #ffffff; text-decoration: none; padding: 14px 35px; border-radius: 50px; font-weight: 700; font-size: 12px; text-transform: uppercase; letter-spacing: 1.5px; display: inline-block; box-shadow: 0 6px 15px rgba(0,0,0,0.12); transition: all 0.2s ease;">${trackButtonText}</a>
                  </td>
                </tr>
    ...
    const info = await sendEmail({
      ...
      html,
      text: `Your Order #${orderIdStr} has been ${status}. Total: PKR ${totalPrice.toLocaleString()}. You can track or view details here: ${baseUrl}/track?orderId=${order._id}`,
    });

    return info;
  } catch (error) {
    console.error(`Error sending ${order?.status} email update:`, error);
    throw error;
  }
};
